;(function (global, factory) {
  ...
  ruit.compose = function () {
    for (
      var _len = arguments.length, tasks = new Array(_len), _key = 0;
      _key < _len;
      _key++
    ) {
  ...
  function ruit() {
    for (
      var _len2 = arguments.length, tasks = new Array(_len2), _key2 = 0;
      _key2 < _len2;
      _key2++
    ) {
  ...
  erre.install = function (name, fn) {
    ...
    if (API_METHODS.has(name)) {
      panic(
        `The ${name} is already part of the erre API, please provide a different name`,
      )
    } else {
      erre[name] = fn
  ...
  function erre() {
    ...
    return Object.assign(stream, {
      ...
      push(input) {
        const { value, done } = stream.next(input) // dispatch the stream events
  ...
  const matchOrSkip = (pathRegExp) => (path) =>
    match(path, pathRegExp) ? path : erre.cancel()
  /**
  ...
  const toRegexp = (path, keys, options) =>
    pathToRegexp(path, keys, mergeOptions(options))
  /**
  ...
  const toPath = (path, params, options) =>
    compile(path, mergeOptions(options))(params)
  /**
  ...
  var routerHoc = {
    ...
    exports: riot.pure(({ slots, attributes, props }) => {
      if (wasInitialized)
        panic$2('Multiple <router> components are not supported')

      const getAttribute = (name) =>
        attributes && attributes.find((a) => a.name === name)
